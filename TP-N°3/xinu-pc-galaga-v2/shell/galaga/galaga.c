...
int pid_juego, pid_puntajeYVida, pid_control;
//objects
struct Players {
...
int galaga(void) {
	...
	pid_control = create(gestorControl, 1024, 20, "Proceso Control", 0);
	resume(pid_control);
	...
	while(1) {
		...
		for (int a = 0; a < 9; a++) {
			...
			if (collision(easyEnemies[a].enemyX, easyEnemies[a].enemyY, 20, 20, player.playerX, player.playerY)) {
				//mandamos el mensaje para restar vidas
		...
		for (int i = 0; i < N_SHOOTS; i++) {
			...
			for (int j = 0; j < 9; j++) {
				if (collision(easyEnemies[j].enemyX, easyEnemies[j].enemyY, 15, 15, shoots[i] % 240, shoots[i] / 240)) {
					...
					shoots[i] = 0;
					//mandamos el mensaje para sumar puntaje
...
void puntajeYVida() {
	...
	while (1) {
    	...
    	print_text_on_vga(10, 220, cadena1);
    	print_text_on_vga(10, 240, cadena2);    
