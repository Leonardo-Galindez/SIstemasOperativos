...
void convertir(int in_fd, BMPInfoHeader infoh)
{
...
int main()
{
	...
	int out_fd = open(GRAYSCALE_FILE, O_WRONLY | O_CREAT | O_TRUNC, 0644); // O_TRUNC si existe se borra el cotenido
	if (out_fd < 0)
	{
		something_wrong(in_fd, "error en output");
	}

	convertir(in_fd, infoh);
