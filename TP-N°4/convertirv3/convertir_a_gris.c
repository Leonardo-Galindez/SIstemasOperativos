...
void convertir(int in_fd, int out_fd, BMPInfoHeader infoh, int inicio, int fin)
{
    ...
    lseek(in_fd, sizeof(BMPHeader) + sizeof(BMPInfoHeader) + inicio * (width * 3 + padding), SEEK_SET); // Move to start line
    lseek(out_fd, sizeof(BMPHeader) + sizeof(BMPInfoHeader) + inicio * (width * 3 + padding), SEEK_SET);
